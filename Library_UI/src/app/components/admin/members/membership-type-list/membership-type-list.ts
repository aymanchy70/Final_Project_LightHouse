...
@Component({
  ...
  styleUrl: './membership-type-list.css'
})
export class MembershipTypeListComponent implements OnInit {
  allItems: any[] = []; filtered: any[] = []; paged: any[] = [];
  searchTerm = ''; sort: SortState = { column: 'membershipTypeId', direction: 'asc' };
  ...
  load() {
    this.loading = true;
    this.adminService.getMembershipTypes().subscribe({
      next: (d: any) => { this.allItems = d; this.refresh(); this.loading = false; this.cdr.markForCheck(); },
      error: () => { this.error = 'Failed to load membership types.'; this.loading = false; this.cdr.markForCheck(); }
    });
  }

  refresh() {
    let d = applySearch(this.allItems, this.searchTerm, ['name', 'description']);
  ...
  edit(id: number) { this.router.navigate(['/admin/membership-types/edit', id]); }
  details(id: number) { this.router.navigate(['/admin/membership-types/details', id]); }
  delete(id: number) {
    if (!confirm('Delete this membership type?')) return;
    this.adminService.deleteMembershipType(id).subscribe({
      next: () => {
        this.successMessage = 'Deleted!';
        this.allItems = this.allItems.filter(i => i.membershipTypeId !== id);
        this.refresh();
