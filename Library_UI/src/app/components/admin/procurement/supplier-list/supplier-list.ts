...
export class SupplierListComponent implements OnInit {
  ...
  searchTerm = ''; sort: SortState = { column: 'supplierId', direction: 'asc' };
  ...
  load() {
    ...
    this.adminService.getSuppliers().subscribe({
      ...
      error: () => { this.error = 'Failed to load suppliers.'; this.loading = false; this.cdr.markForCheck(); }
    });
  }
  ...
  edit(id: number) { this.router.navigate(['/admin/suppliers/edit', id]); }
  details(id: number) { this.router.navigate(['/admin/suppliers/details', id]); }
  delete(id: number) {
    if (!confirm('Delete this supplier?')) return;
    this.adminService.deleteSupplier(id).subscribe({
      next: () => {
        this.successMessage = 'Deleted!';
        this.allItems = this.allItems.filter(i => i.supplierId !== id);
        this.refresh(); this.cdr.markForCheck();
